function loadFoodItems(){
    ...
    if(foodCollection!=null){
        ...
        foodItemsTable.innerHTML='<tr><th>Food Item Image</th><th>Food Item Name</th><th>Available in Restaurants</th><th>Rate</th><th>Quanity</th><th>Price</th></tr>';
        if(foodCollection !=null){
            for(var foodItemId in foodCollection){
                ...
                '"><img src='+foodItem.image+' width="75px" height="75px"></a></td>'+'<td class="fooditems" id='+foodItem.id+'><a href="select_restaurant.html?fooditem='+foodItem.id+'">'
                +foodItem.title+'</td><td>'+optnText+'</a></td>'+'<td><input type="number" id="rate'+foodItem.id+'" class="foodrate" disabled></td>'+'<td><input type="number" id="qty'+foodItem.id+'" class="foodqty" min="0" max="10"></td>'+'<td><input type="number" id="price'+foodItem.id+'" class="foodPrice" onClick=getPriceByRestaurant() disabled></td></tr>'
